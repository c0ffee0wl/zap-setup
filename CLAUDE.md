...
**Do not re-add `-Wait` to the installer `Start-Process`.** Zap's Inno `[Run]` entry auto-launches the app on completion *without* the `skipifsilent`/`nowait` flags (verified empirically — Zap opens during `/VERYSILENT`), so `[Run]` is **not** suppressible from the command line and a blocking `-Wait` would hang until the user closes the Zap window. The launch is cosmetic — `[Run]` is the last phase, after all `[Files]` are copied, so it creates nothing the installer didn't already write. `Install-Zap` therefore starts the installer with `-PassThru` (no `-Wait`), then loops: once `Get-ZapInstallDir` resolves (the uninstall key is written *before* `[Run]`, so this means files are down), it closes the spawned Zap — matched by `MainWindowHandle != 0` **and** a path under the install dir **and** a `StartTime` later than `$startedAt` (captured via `Get-Date` *before* launch — never `$proc.StartTime`, which throws on a fast/elevated process under `ErrorActionPreference='Stop'`). The close is deliberately **not** gated on a version-string match: Inno's `DisplayVersion` may carry the tag's leading `v` while `$rel.Version` strips it (the version short-circuit at the top of `Install-Zap` normalizes both with `-replace '^[vV]',''` for the same reason). A 5-min deadline bounds the loop; if the installer still hasn't exited (a Zap window we couldn't match), it's `Stop-Process`'d and we proceed, since the files are already installed. Success is judged by a registered uninstall key, **not** the exit code — force-closing the `[Run]` child must not turn a completed install into a fatal `Write-Err`.
