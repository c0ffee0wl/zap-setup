...
All three provider blocks — these two plus the Windows Azure injection — deliberately agree on `api_type = "open_ai_resp"` (Zap's Responses adapter) and on one model, the mini-tier member of the current GPT-5.x family. `linux/configs/settings.toml` is the reference copy for the model's `id`, `context_window`, `max_output_tokens` and capability flags; bump all three together, and each block carries a comment saying so. The LiteLLM block deviates in the model's `id` **and** `name` (it keeps the proxy's `azure/` routing prefix — don't "normalize" that away); it is also the one that may need `api_type = "open_ai"` back, since a proxy build without the `/v1/responses` route 404s.
