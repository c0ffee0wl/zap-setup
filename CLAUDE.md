...
Several blocks are **copied character-for-character** from the linux-setup script, each annotated with `# verbatim from linux-setup.sh:NNN-MMM` (line ranges refer to upstream v2.18.0). Shared helpers (colors, `log`/`warn`/`error`, `backup_file`, `prompt_yes_no`, `apt_get`) live in `linux/common.sh`; the remaining lifts (the `LC_ALL`/`LANG=C.UTF-8` locale exports, arg parsing, root + Debian preflight, Phase 0 self-update) live in `linux/setup.sh`. Do **not** refactor or "improve" these — the whole point is that they evolve in lockstep with the upstream helper script, in both directions: don't "fix" a lifted quirk upstream still has, and when upstream changes a lifted block, adopt the new form here and refresh the annotations (as done when re-syncing the lifts against v2.18.0, which picked up the TTY/`NO_COLOR`-conditional colors and the `git rev-list --count` self-update counter that had accumulated upstream — the latter replacing the old `wc -l || echo "0"` quirk). `linux/common.sh` is a hosting location, not a refactoring layer.
